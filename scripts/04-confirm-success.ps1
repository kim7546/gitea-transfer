...
try {
    ...
    $freezeCommit = Get-TagTarget $freezeTag

    $claimTag = "$claimPrefix$freezeId"
    if (!(Get-RemoteTagExists $remote $claimTag)) {
        throw "Claim Tag가 없습니다. 반입담당자가 Claim & Export를 수행했는지 확인하세요."
    }
    ...
    if (Get-RemoteTagExists $remote $successTag) {
        Fetch-OneTag $remote $successTag
        $target = Get-TagTarget $successTag
        if ($target -eq $freezeCommit) {
            Write-Host "이미 Success 처리된 Freeze입니다: $successTag"
            return
        }
        throw "동일 FreezeId의 Success Tag가 다른 Commit을 가리킵니다."
    }
...
ConfirmedAt=$((Get-Date).ToString("yyyy-MM-ddTHH:mm:ssK"))
ClaimInfo:
$claimMessage
"@

    & git tag -a $successTag $freezeCommit -m $msg
    Assert-GitSuccess "Success Tag 생성 실패"

    & git push $remote "refs/tags/$successTag"
    Assert-GitSuccess "Success Tag Push 실패"

    # 완료된 작업의 임시 Claim Tag는 제거한다. Freeze/Success Tag는 이력으로 보관한다.
    & git push $remote ":refs/tags/$claimTag"
    Assert-GitSuccess "완료 후 Remote Claim Tag 삭제 실패"
    ...
    Write-Host "Tag     : $successTag"
    Write-Host "다음 반입은 이 Freeze 이후부터 시작합니다."
}
finally {
    Pop-Location
}
